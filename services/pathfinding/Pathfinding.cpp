...
const double SWEEP_WIDTH =
    3; // Increased spacing for visibility during testing, can be reduced to 1.2 later
...
void Pathfinding::setMissionData(const QVariantList& missionPoints,
                                 const QVariantList& restrictionZones) {
    m_missionPoints = missionPoints;
    m_restrictionZones = restrictionZones;
    qDebug() << "[Pathfinding] Données reçues: " << missionPoints.size() << "points mission,"
             << restrictionZones.size() << "zones restriction.";
}
...
Pathfinding::GeoCoord Pathfinding::variantToGeo(const QVariant& item) const {
    if (item.canConvert<QGeoCoordinate>()) {
        ...
        double lat = map.contains("latitude") ? map["latitude"].toDouble()
                                              : (map.contains("lat") ? map["lat"].toDouble() : 0.0);
        double lng = map.contains("longitude")
                         ? map["longitude"].toDouble()
                         : (map.contains("lng") ? map["lng"].toDouble() : 0.0);
        return {lat, lng};
    }
    return {0, 0};
}
...
QPainterPath Pathfinding::buildPathFromVariantList(const QVariantList& list,
                                                   const GeoCoord& referenceCoord) const {
    QPainterPath path;
    ...
    if (isNested) {
        for (const QVariant& zoneVar : list) {
            ...
            for (int i = 0; i < sublist.size(); ++i) {
                ...
                if (i == 0)
                    subPolygon.moveTo(local.x, local.y);
                else
                    subPolygon.lineTo(local.x, local.y);
            }
...
QList<QPainterPath> Pathfinding::subdivideSafeRegions(const QPainterPath& safeArea) const {
    ...
    for (const QPolygonF& poly : polys) {
        ...
        if (safeArea.contains(centroid)) {
            ...
            qDebug() << "[SUBDIVISION] Polygone ignoré (zone de restriction détectée, centroide "
                        "hors zone sûre)";
        }
    }
...
bool Pathfinding::isLineBlocked(const QPointF& p1, const QPointF& p2,
                                const QPainterPath& restr) const {
    if (restr.isEmpty())
        return false;
    ...
    // 2. Sampling check for lines strictly inside the restriction (clipping inside a corner)
    QPointF mid = (p1 + p2) / 2.0;
    if (restr.contains(mid)) {
        // Since Qt contains() returns true for points exactly on the boundary,
        // we must check if this point is sliding on the boundary, or strictly inside.
        bool onBoundary = false;
        for (const QPolygonF& poly : polys) {
            for (int i = 0; i < poly.size(); ++i) {
                QPointF a = poly[i];
                QPointF b = poly[(i + 1) % poly.size()];
                double l2 = qPow(a.x() - b.x(), 2) + qPow(a.y() - b.y(), 2);
                if (l2 < 1e-9) continue;
                double t = qMax(0.0, qMin(1.0, QPointF::dotProduct(mid - a, b - a) / l2));
                QPointF proj = a + t * (b - a);
                double d2 = qPow(mid.x() - proj.x(), 2) + qPow(mid.y() - proj.y(), 2);
                if (d2 < 0.001) { // Grazing tolerance (sqrt(0.001) ~ 3.1cm)
                    onBoundary = true;
                    break;
                }
            }
        ...
        if (!onBoundary) return true; // Strictly inside the polygon
    }

    return false;
}

QList<Pathfinding::Point2D> Pathfinding::findSafePath(const QPointF& p1, const QPointF& p2,
                                                      const QPainterPath& restr,
                                                      const QList<QPolygonF>& polys) const {
    QList<Point2D> waypoints;
    if (!isLineBlocked(p1, p2, restr))
        return waypoints;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int v = 0; v < n; ++v) {
            if (visited[v] || u == v)
                continue;
            if (!isLineBlocked(nodes[u], nodes[v], restr)) {
                QPointF dVec = nodes[u] - nodes[v];
                double weight = qSqrt(dVec.x() * dVec.x() + dVec.y() * dVec.y());
        ...
        if (waypoints.size() > 1) {
            ...
            for (int i = 0; i < waypoints.size(); ++i) {
                ...
                if (i + 1 < waypoints.size()) {
                    if (!isLineBlocked(currentStart,
                                       QPointF(waypoints[i + 1].x, waypoints[i + 1].y), restr)) {
...
QList<Pathfinding::GeoCoord> Pathfinding::computeGrid(const QPainterPath& operableArea,
                                                      const QList<QPolygonF>& restrPolys,
                                                      double angle,
                                                      const GeoCoord& refCoord) const {
    QList<GeoCoord> finalPath;
    if (operableArea.isEmpty())
        return finalPath;
    ...
    QRectF initialBounds = rotSafe.boundingRect();
    
    // 2.5 Bisection Barrier: Split the safe area vertically at each restriction
    // This perfectly isolates Left and Right topological lobes, forcing Boustrophedon 
    // to sweep one full side before seamlessly jumping to the other across the top.
    QPainterPath rotRestr = tfm.map(unionRestr);
    for (const QPolygonF& poly : rotRestr.toSubpathPolygons()) {
        if (poly.size() < 3) continue;
        double midX = poly.boundingRect().center().x();
        QPainterPath barrier;
        // 1cm thick vertical slice through the entire operational space
        barrier.addRect(midX - 0.005, initialBounds.top() - 1000, 0.01, initialBounds.height() + 2000);
        rotSafe = rotSafe.subtracted(barrier).simplified();
    }
    ...
    while (y <= bounds.bottom()) {
        ...
        for (int i = 0; i + 1 < intersections.size(); i += 2) {
            if (intersections[i + 1] - intersections[i] > 0.1) {
                segments.append(
                    {idCounter++, intersections[i], intersections[i + 1], y, false, {}});
            }
        }
        y += SWEEP_WIDTH;
    }
    ...
    for (int i = 0; i < segments.size(); ++i) {
        for (int j = i + 1; j < segments.size(); ++j) {
            ...
            if (qAbs(dy - SWEEP_WIDTH) < 1e-3) {
                ...
                if (qMax(segments[i].x1, segments[j].x1) <=
                    qMin(segments[i].x2, segments[j].x2) + 1e-3) {
    ...
    while (currentId != -1) {
        ...
        if (hasPos) {
            ...
            // If jump is > SWEEP_WIDTH or the transition is blocked by a restriction, use safe
            // routing
            if (qAbs(currentPos.y() - seg.y) > SWEEP_WIDTH + 1e-3 ||
                isLineBlocked(p1, p2, restrUnionLocal)) {
        ...
        if (nextId == -1) {
            for (int i = 0; i < segments.size(); ++i) {
                if (!segments[i].visited) {
                    double d1 = qSqrt(qPow(exitNode.x() - segments[i].x1, 2) +
                                      qPow(exitNode.y() - segments[i].y, 2));
                    double d2 = qSqrt(qPow(exitNode.x() - segments[i].x2, 2) +
                                      qPow(exitNode.y() - segments[i].y, 2));
                    double d = qMin(d1, d2);
                    if (d < minD) {
                        minD = d;
...
QList<Pathfinding::GeoCoord> Pathfinding::computeSubdivision(const QPainterPath& operableArea,
                                                             const QList<QPolygonF>& restrPolys,
                                                             const GeoCoord& refCoord) const {
    // The upgraded computeGrid inherently performs advanced Cell Decomposition via
    // adjacency graphs. Running it at 0 degrees corresponds perfectly to the optimal subdivision
    // approach.
...
void Pathfinding::runVrxSimulation() {
    ...
    QString baseEnv =
        "cd ~/vrx_ws && source /opt/ros/jazzy/setup.bash && source install/setup.bash && ";
    ...
    QtConcurrent::run([this, baseEnv, savePath]() {
        ...
        if (!m_isSimInitialized) {
            ...
            launchCommand(baseEnv +
                          "ros2 launch vrx_gz vrx_environment.launch.py world:=sydney_regatta");
            QThread::msleep(6000);

            // 2) Spawnear WAM-V con bridges ROS2 (IMPORTANTE: usar full)
            launchCommand(baseEnv + "ros2 launch vrx_gz spawn.launch.py world:=sydney_regatta "
                                    "sim_mode:=full name:=wamv model:=wam-v");
            QThread::msleep(3000);
        ...
        launchCommand(
            baseEnv +
            "ros2 launch gps_waypoints fast_uav_waypoints.launch.py checkpoints_file:=" + savePath);
...
void Pathfinding::calculateBestRoute() {
    ...
    auto evaluate = [&](const QString& name, const QList<GeoCoord>& path) {
        ...
        if (dist > 1.0)
            results.append({name, path, dist, time});
    };
    ...
    for (const Result& r : results) {
        ...
        if (r.dist > 1.0 && (best.dist == 0 || r.dist < best.dist))
            best = r;
    }
