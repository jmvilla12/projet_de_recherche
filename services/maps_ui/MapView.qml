...
Item {
    ...
    Map {
        ...
        // Multi-colored segmented paths for Cellular Decomposition visualization
        MapItemView {
            model: root.cellPathsArray
            delegate: MapPolyline {
                line.width: 2.5
                line.color: modelData.color
                opacity: 0.85
                z: 19
                path: modelData.pathCoords
            }
        }
    ...
    function updateGeneratedPathVisuals() {
        ...
        var currentCellId = -1;
        var currentCoords = [];
        var colors = ["#E91E63", "#2196F3", "#4CAF50", "#FF9800", "#9C27B0", "#00BCD4", "#8BC34A", "#FFC107", "#3F51B5"];

        for (var i = 0; i < generatedPath.length; i++) {
            var pt = generatedPath[i];
            var cid = pt.cell_id !== undefined ? pt.cell_id : 1;
            var coord = QtPositioning.coordinate(pt.latitude, pt.longitude);
            
            pathCoords.push(coord);

            if (cid !== currentCellId) {
                if (currentCoords.length > 0) {
                    var c = colors[currentCellId % colors.length];
                    cells.push({ "color": c, "pathCoords": currentCoords });
                }
                currentCellId = cid;
                currentCoords = [];
                if (i > 0) {
                    var prevPt = generatedPath[i - 1];
                    currentCoords.push(QtPositioning.coordinate(prevPt.latitude, prevPt.longitude));
                }
            }
